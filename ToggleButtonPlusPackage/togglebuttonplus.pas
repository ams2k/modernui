...
begin
  ...
  if (FAnimPos < 0) or (FAnimPos > 100) then begin
    FAnimTimer.Enabled := False;
...
begin
  case FStyle of
    mbsPrimary:
      begin
        FToggleColorOn := clBlue; // HTMLColorToRGB('#00aaff');
        FToggleColorOff := clGray;
        FToggleColorArea := clSkyBlue;
        FToggleBorder := True;
        FToggleBorderColor := HTMLColorToRGB('#0085c8');
        FToggleBorderStyle := psSolid;
        FBallCircleColor := HTMLColorToRGB('#7d7d7d');
    ...
    mbsSuccess:
      begin
        FToggleColorOn := HTMLColorToRGB('#47f53b'); //clLime;
        FToggleColorOff := clGray;
    ...
    mbsDanger:
      begin
        FToggleColorOn := HTMLColorToRGB('#c34100');
        FToggleColorOff := clGray;
        FToggleColorArea := clWhite;
        FToggleBorder := True;
        FToggleBorderColor := HTMLColorToRGB('#a7322a');;
        FToggleBorderStyle := psSolid;
        FBallCircleColor := HTMLColorToRGB('#7d7d7d');
...
begin
  ...
  Canvas.Font := Font;
  ToggleAreaHeight := 20;
  lTop := (Height - ToggleAreaHeight) div 2 + 1;
  lLeft := 3;
  Radius := (ToggleAreaHeight div 2) + 2;
  ...
  if FChecked then
    FBallColor := FToggleColorOn
  else
    FBallColor := FToggleColorOff;
  ...
  lRight := lLeft + SwitchWidth - BallSize - (BallSize div 2) - 1; // Right da bolinha
  ...
  Canvas.Pen.Color := FBallCircleColor;
  Canvas.Pen.Width := 1;
  Canvas.Ellipse(BallPos, lTop, BallPos + BallSize, lTop + BallSize);
