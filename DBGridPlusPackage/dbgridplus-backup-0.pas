...
type
  TDBGridPlusStyle = (gsDefault, gsModern, gsClassic);
  ...
  published
    ...
    property GridStyle: TDBGridPlusStyle read FGridStyle write SetGridStyle default gsClassic;
...
begin
  ...
  SelectedRowColor := TColor($5ed9ff);// HTMLColorToRGB('#5ed9ff');
  SelectedRowFontColor := clBlue;
  FGridStyle := gsClassic;
...
begin
  ...
  try
    if IsFixed and IsModern then begin
      ...
      if (ACol >= 0) and (ACol < Columns.Count) and Assigned(Columns[ACol].Field) then begin
...
begin
  ...
  FGridRows := AValue;
  Height := DefaultRowHeight * AValue + DefaultRowHeight - 1;
  invalidate;
end;
