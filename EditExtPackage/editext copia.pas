...
type
  TEditExtOption = (NORMAL, CPF, CNPJ, CEP, TELEFONE, TITULO);
  { TEditExt }
  ...
  private
    ...
    procedure SetOption(AValue: TEditExtOption);
    ...
    function FormataSpecialEdit(txt: string): string;
  protected
    procedure Click; override; // Sobrescreve o método Click
  public
    constructor Create(AOwner: TComponent); override;
    function TextNotFormated: string;
  published
    ...
    property SpecialEdit: TEditExtOption read FOption write SetOption;
...
function TEditExt.TextNotFormated: string;
//retorna o texto sem formata
begin
  if FOption <> NORMAL then
    Result := RemoveFormatacao(Text)
  else
    Result := Text;
end;
...
procedure TEditExt.SetOption(AValue: TEditExtOption);
begin
  if FOption = AValue then Exit;
  if FIsCurrency then Exit;
  FOption := AValue;
  NumbersOnly := True;
  FCurSymbol := '';
  Text := FormataSpecialEdit(Text);
  Invalidate;
end;
...
begin
  ...
  else if FOption<>NORMAL then
    Result := FormataSpecialEdit(Text)
  else
    Result := Text;
end;
...
begin
   ...
   else if FOption<>NORMAL then begin
     case FOption of
       CPF: MaxLength := 14;
       CNPJ: MaxLength := 18;
       CEP: MaxLength := 9;
...
function TEditExt.FormataSpecialEdit(txt: string): string;
// Formata para: CPF, CNPJ, CEP, TELEFONE, TITULO
var
  msk: string;
  iMask, iDigit: Integer;
begin
  if (FOption = NORMAL) or FIsCurrency then begin
    Result := txt;
    Exit;
  end;
  ...
  Result := txt;
  msk := '';
  txt := RemoveFormatacao(txt);
  txt := txt.Replace('-', '');

  case FOption of
    CPF:
      begin
        txt := RightStr('00000000000' + txt.Trim, 11);
        msk := '###.###.###-##';
      end;
    CNPJ:
      begin
        txt := RightStr('00000000000000' + txt.Trim, 14);
        msk := '##.###.###/####-##';
      end;
    CEP:
      begin
        txt := RightStr('00000000' + txt.Trim, 8);
        msk := '#####-###';
      end;
    TITULO:
      begin
        txt := RightStr('0000000000' + txt.Trim, 10);
        msk := '########/#-#';
      end;
    TELEFONE:
      begin
        if txt.Length < 8 then txt := '00000000';
        if txt.Length <= 8 then begin
          txt := RightStr('00000000' + txt.Trim, 8);
          msk := '####-####';
        end
        else if txt.Length = 9 then begin
          txt := RightStr('000000000' + txt.Trim, 9);
          msk := '#####-####';
        end
        else if txt.Length = 10 then begin
          txt := RightStr('0000000000' + txt.Trim, 10);
          msk := '(##) ####-####';
        end
        else if txt.Length = 11 then begin
          txt := RightStr('00000000000' + txt.Trim, 11);
          msk := '(##) #####-####';
        end
        else begin
          txt := RightStr('000000000000000' + txt.Trim, 8);
          msk := '####-####';
        end;
      end;
  end;
  ...
  if msk <> '' then begin
    iDigit := 1;
    Result := '';
    for iMask := 1 to Length(msk) do begin
      if (msk[iMask] = '#') then begin
        if iDigit <= Length(txt) then begin
          Result := Result + txt[iDigit];
          Inc(iDigit);
        end else
          Break; // Sem mais dígitos para preencher
      end else
        Result := Result + msk[iMask];
    end;
  end;
end;
...
begin
  ...
  if FIsCurrency or (FOption<>NORMAL) then
  begin
